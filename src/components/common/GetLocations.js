...
export default function GetLocations({navigation}) {
  ...
  const getLocations = () => {
    console.log(accessToken,"Fetching data");
    axios
      .get(GET_LOCATIONS, {
        headers: {
      ...
      .then(function (response) {
        ...
        console.log("Done");
        console.log('Data has arrived', response.data.locations);
      })
      .catch(function (error) {
        console.error(error, 'Get Locations');
      });
  };

  const addToRequestData = (val,_id) => {
      ...
      <FlatList
        ...
        renderItem={({item}) => (
          <Location
            ...
            onClick={() => {
              addToRequestData(item.unit_or_building,item._id);
